...
def main () :
    ...
    while user_choice != "" :
        if user_choice == '1' :
            ...
            cipher_lng = enc.get_cipher_lng(user_text, cipher_key)
            char_coord = enc.get_char_coord(cipher_key, user_text, cipher_lng)
            char_shift = enc.get_char_shift(cipher_key)
            encypted_char_value = enc.get_cipher_text_value(user_text, char_shift, cipher_key)
            cipher = enc.encrypt(encypted_char_value, char_coord, cipher_key, cipher_lng)
